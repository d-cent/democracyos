...
UserProfile.prototype.shouldShowEdition = function (userId) {
  if (citizen.id === userId) {
    ...
    o('#user-profile-wrapper .profile-container h2', this.el).on('click', function () {
      self.toggle();
      self.editView.toggle();
    });
    o('.about-container', this.el).on('click', function () {
      self.toggle();
      self.editView.toggle();
    });
    o('#user-profile-wrapper .profile-container', this.el).on('click', function () {
      self.toggle();
      self.editView.toggle();
    });
