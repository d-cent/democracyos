...
FilterView.prototype.switchOn = function() {
  ...
  this.events.bind('click #tag-filter ul li', 'onfilterclick');
...
FilterView.prototype.onfilterclick = function(ev) {
  ev.preventDefault();

  var target = ev.delegateTarget || closest(ev.target, '[data-tag]', true);
  var tag = tags.get(target.getAttribute('data-tag'));

  tag = tag ? tag : filter.defaultTag();
